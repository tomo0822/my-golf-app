export default function SetupScreen({
  ...
  return (
    <div className="p-6 min-h-screen bg-slate-950 text-white flex flex-col items-center">
      <h1 className="text-4xl font-black text-center mb-6 text-transparent bg-clip-text bg-gradient-to-b from-green-400 to-green-700">
        OLYMPIC
      </h1>
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-6 rounded-[2rem] shadow-2xl space-y-6">
        <div className="flex justify-between items-center">
          <p className="text-xl font-black">メンバー登録</p>
          {players.length < 4 && (
            <button
              onClick={addPlayer}
        ...
        <div className="space-y-3">
          {players.map((name, i) => (
            <div key={i} className="flex gap-2">
              <input
                className="flex-1 bg-slate-800 border border-slate-700 p-4 rounded-2xl font-bold outline-none focus:border-green-500"
                value={name}
              ...
              {players.length > 2 && (
                <button
                  ...
                  className="bg-slate-800 w-12 rounded-2xl border border-slate-700"
                >
                  ✕
                </button>
              )}
        ...
        <button
          onClick={onStart}
          className="w-full bg-gradient-to-b from-green-500 to-green-700 py-4 rounded-2xl font-black text-lg"
        >
          START GAME
        </button>
      </div>
